...
public class TransactionAcknowledgeActionImpl implements TransactionAcknowledgeAction {
  ...
  public void acknowledgeTransaction(ServerTransactionID stxID) {
    try {
      ChannelID channelID = stxID.getChannelID();
      ...
      try {
        if (transactionBatchManager.batchComponentComplete(channelID, stxID.getClientTransactionID())) {
          BatchTransactionAcknowledgeMessage msg = channelManager.newBatchTransactionAcknowledgeMessage(channelID);
