...
public class TCWorkerCommManager {
  ...
  private static final TCLogger   lossyLogger        = new LossyTCLogger(logger, 1, LossyTCLogger.COUNT_BASED, false);
  ...
  private final int WEIGHT_BASED = 1;

  public synchronized CoreNIOServices getNextWorkerComm() {
    CoreNIOServices[] leastWeightWorkerComms = getWorkerCommsBy(WEIGHT_BASED, workerCommThreads);
    CoreNIOServices rv;
  ...
  private CoreNIOServices[] getWorkerCommsBy(int mode, CoreNIOServices[] workerComms) {
    ...
    for (int i = 0; i < workerComms.length; i++) {
      ...
      if (mode == WEIGHT_BASED) {
        presentValue = workerComms[i].getWeight();
      } else {
        throw new AssertionError("Unknown method to select to worker comms");
      }
