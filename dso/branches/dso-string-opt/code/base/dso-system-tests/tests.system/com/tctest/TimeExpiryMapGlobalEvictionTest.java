...
public class TimeExpiryMapGlobalEvictionTest extends ServerCrashingTestBase {
  ...
  protected void createConfig(TerracottaConfigBuilder cb) {
    ...
    lock5.setMethodExpression(" * com.tcclient.cache.GlobalKeySet.*(..)");
    setLockLevel(lock5);
    
    LockConfigBuilder lock6 = new LockConfigBuilderImpl(LockConfigBuilder.TAG_AUTO_LOCK);
    lock6.setMethodExpression(" * com.tcclient.cache.GlobalKeySet.allGlobalKeys(..)");
    setReadLockLevel(lock6);
    
    LockConfigBuilder lock7 = new LockConfigBuilderImpl(LockConfigBuilder.TAG_AUTO_LOCK);
    lock7.setMethodExpression("* " + barrierClassName + "*.*(..)");
    setLockLevel(lock7);
    
    LockConfigBuilder lock8 = new LockConfigBuilderImpl(LockConfigBuilder.TAG_AUTO_LOCK);
    lock8.setMethodExpression(" * com.tcclient.cache.CacheParticipants.*(..)");
    setLockLevel(lock8);
    
    LockConfigBuilder lock9 = new LockConfigBuilderImpl(LockConfigBuilder.TAG_AUTO_LOCK);
    lock9.setMethodExpression(" * com.tcclient.cache.CacheParticipants.getCacheParticipants(..)");
    setReadLockLevel(lock9);
    
    LockConfigBuilder lock10 = new LockConfigBuilderImpl(LockConfigBuilder.TAG_AUTO_LOCK);
    lock10.setMethodExpression(" * com.tcclient.cache.CacheParticipants.getNodeId(..)");
    setReadLockLevel(lock10);
    
    cb.getApplication().getDSO().setLocks(new LockConfigBuilder[] { lock1, lock2, lock3, lock4, lock5, lock6, lock7, lock8, lock9 });
    ...
    instrumented5.setClassExpression("com.tcclient.cache.*");
    
    InstrumentedClassConfigBuilder instrumented6 = new InstrumentedClassConfigBuilderImpl();
    instrumented6.setClassExpression("com.tcclient.ehcache.*");
    
    InstrumentedClassConfigBuilder instrumented7 = new InstrumentedClassConfigBuilderImpl();
    instrumented7.setClassExpression(barrierClassName + "*");
    
    InstrumentedClassConfigBuilder instrumented8 = new InstrumentedClassConfigBuilderImpl();
    instrumented8.setClassExpression("com.tcclient.cache.CacheDataStore");
    instrumented8.setHonorTransient(true);
    instrumented8.setCallMethodOnLoad("initialize");
    
    cb.getApplication().getDSO().setInstrumentedClasses(
                                                        new InstrumentedClassConfigBuilder[] { instrumented1,
                                                            instrumented2, instrumented3, instrumented4, instrumented5, instrumented6, instrumented7, instrumented8 });
