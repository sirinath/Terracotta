  ...
  def postscript(ant, build_environment, product_directory, *args)
    ...
    unless @static_resources.docflex_home.nil?
      ...
      ant.java(
        ...
        ant.arg(:line => "-p:docTitle=\"Terracotta Configuration Schema Version 2.2\"")
        ant.arg(:line => "-d #{tmpdir.to_s}")
        ant.arg(:line => "-f tc-config-v2.2")
      ...
      File.open(index_file.to_s, 'w') do |out|
        out.puts "<html>"
        out.puts " <head>"
        out.puts "   <META HTTP-EQUIV=\"REFRESH\" CONTENT=\"0; URL=tc-config-v2.2.html\">"
        out.puts " </head>"
        out.puts "</html>"
      end
    end
