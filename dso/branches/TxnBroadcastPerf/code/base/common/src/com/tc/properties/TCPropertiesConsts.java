...
public interface TCPropertiesConsts {
  ...
  /*********************************************************************************************************************
   * Section : OLD TC Properties The old properties which were present earlier and now got removed If any of the
   * property is renamed/deleted then make sure that u add that in this section
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : L2 Object Manager Properties Description : This section contains the defaults for the object manager of
   * the L2 cachePolicy : <lru>/<lfu> - Least Recently Used or Least Frequenctly used deleteBatchSize - Max number of
   * objects deleted in one transaction when removing from the object store after a GC maxObjectsToCommit - Max numberof
   * Objects commited in one transaction in the commit stage and flush stage maxObjectsInTxnObjGrouping - Max number of
   * Objects allowed in the TransactionalObject grouping maxTxnsInTxnObjectGrouping - Max number of Transations allowed
   * in the TransactionalObject grouping fault.logging.enabled - Enables/Disables logging of ManagedObject Faults from
   * disk. If enabled, it logs every 100 faults. loadObjectID.fastLoad - Enables/Disables fast loading of ObjectIDs.
   * Only effective for persistence with mode permanent-store. This will speed up Object-Ids loading at restart but some
   * overhead occurred at regular operations. You can go from enable to disable but need a fresh start if change from
   * disable to enable for building up compressed object-Id. loadObjectID.longsPerDiskEntry - Size of long array entry
   * to store object IDs in persistent store. One bit for each ID. loadObjectID.checkpoint.changes - number of changes
   * to trigger objectID checkpoint loadObjectID.checkpoint.maxlimit - max number of changes to process in one run
   * checkpoint. loadObjectID.checkpoint.timeperiod - time period in milliseconds between checkpoints
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : L2 Seda stage properties Description : This section contains configuration for SEDA stages for L2
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : L2 Berkeley DB Persistence Layer Properties Description : This section contains the of Berkeley DB JE
   * properties thats used in L2 For an explanation of these properties look at Berkeley DB documentation (l2.berkeleydb
   * is removed before giving to Berkeley DB JE) je.lock.timeout = 3mins (in microsecs) since je throw Deadlock
  ...
  /*********************************************************************************************************************
   * Section : L2 LFU cachepolicy defaults Description : If cachePolicy is set to lfu, then these values take effect
   * agingFactor (float) - valid values 0 to 1 recentlyAccessedIgnorePercentage (int) - valid values 0 - 100
  ...
  /*********************************************************************************************************************
   * Section : L2 Bean shell Properties Description : Bean shell can be enabled in the server for debugging.
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : Network HA (nha) Description : If Networked HA is enabled then these values take effect groupcomm.type -
   * communication layer can be "tc-group-comm" or "tribes" tcgroupcomm.handshake.timeout - tc-group-comm handshake
   * timeout milliseconds tcgroupcomm.response.timelimit - tc-group-comm message response timelimit millisecon
   * RuntimeException thrown after timelimit tcgroupcomm.discovery.interval - tc-group-comm member discovery interval
   * milliseconds send.timeout.millis - number of milliseconds to retry sending a message tribes.failuredetector.millis -
   * number of milliseconds for a node to response a check otherwise will be removed from group. mcast.enabled - If
   * true, uses Multicast instead of TCP for L2-L2 discovery l2.nha.tribes.mcast.* - these properties are passed to
   * tribes
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : L1 JVM Compatibility Properties Description : This section contains the defaults for the JVM
  ...
  /*********************************************************************************************************************
   * Section : L1 Integration Modules Description : This section contains the defaults for the L1 integration modules
   * repositories - comma-separated list of additional module repositories URL's; if the tc.install-root system property
   * is set, a default repository of (tc.install-root)/modules will be injected default - comma-separated list of
   * integration modules that are implicitly loaded by the L1 in the form specified by the Required-Bundles OSGI
   * manifest header additional - list of additional integration modules to be started, in the form specified by the
   * OSGI Required-Bundles manifest header tc-version-check - off|warn|enforce|strict
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : L1 Cache Manager Properties Description : This section contains the defaults for the cache manager for
  ...
  /*********************************************************************************************************************
   * Section : L1 Transaction Manager Properties Description : This section contains the defaults for the Transaction
   * manager for the L1 logging.enabled - if true, enables some logging in the transaction manager
   ...
   * maxPendingBatches - the max number of pending batches the client creates before a Batch ack is received from the
   * server, after which the client stalls until a Batch ack is received. maxSleepTimeBeforeHalt - the max time that a
   * user thread will wait for L2 to catchup if the L2 is behind applying transactions. This time is used before
   * maxPendingBatches is reached. The units are in milliseconds completedAckFlushTimeout - the timeout in milliseconds
   * after which a NullTransaction is send to the server if completed txn acks are still pending
   * strings.compress.enabled - Enables string compression when sending to the L2. There is a processing overhead at the
   * L1, but saves network bandwidth, reduces memory requirements in the L2 and also reduces disk io at the L2.
   * strings.compress.minSize - Strings with lengths less that this number are not compressed folding.enabled -
   * true/false whether txn folding is enabled. Folding is the act of combining similar (but unique) application
   * transactions into a single txn (for more optimal processing on the server). Only transactions that share common
   * locks and objects can be folded. folding.lock.limit - the maximum number of distinct locks permitted in folded txns
   * (0 or less means infinite) folding.object.limit - the maximum number of distinct objects permitted in folded txns
  ...
  /*********************************************************************************************************************
   * Section: L1 Connect Properties Description: This section contains properties controlling L1 connect feature
   * max.connect.retries - maximum L2 connection attempts connect.versionMatchCheck.enabled - if true, connection is
   * established only when L1 and L2 are of the same DSO version
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section: L1 Reconnect Properties Description: This section contains properties controlling L1 reconnect feature
   * Note that l1 get these properties from l2, so the local copy of l1 doesn't matter enabled - if true, enables l1
   * reconnect feature (and Once-And-Only-Once protocol) timeout.millis - number of milliseconds a disconnected L1 is
   * allowed to reconnect to L2 that has not crashed
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : L1 Object Manager Properties Description : This section contains the defaults for the Object manager for
   * the L1
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : Common Logging properties for both L1 and L2 Description : Logging attributes that can be overridden.
   * maxBackups - The maximum number of backup log files to keep maxLogFileSize - The maximum size of a log file in
   * megabytes
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : Common Stage Monitoring properties for both L1 and L2 Description : Stage monitoring can be enabled or
   * disabled for debugging. enabled : <true/false> - Enable or Disable Monitoring delay : long - frequency in
   * milliseconds
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : Session properties (applies to all DSO session enabled web apps in this VM) id.length : The length (in
   * chars) for session identifiers (min 8) serverid : The server identifier to place in the session ID delimiter :
   * Thedelimiter that separates the server ID from the session ID cookie.domain : Domain value for session cookie
   * cookie.secure : Enable / disable the secure flag in the session cookie cookie.maxage.seconds : The maximum lifetime
   * of the session cookie cookie.name : Name of the session cookie cookie.enabled : Enable / disable the use of cookies
   * for session tracking maxidle.seconds : Session idle timeout in seconds tracking.enabled : Enable / disable session
   * tracking completely urlrewrite.enabled : Enable / disable the URL functionality attribute.listeners : Comma
   * separated list of HttpSessionAttributeListener classes listeners : Comma separated list of HttpSessionListener
   * classes invalidator.sleep : Sleep time between runs of the session invalidator request.bench.enabled : Enable /
   * disable request benchmark logging invalidator.bench.enabled : Enable / disable benchmark logging for session
   * invalidation request.tracking : Enable / disable the stuck request monitor request.tracking.dump : Enable / disable
   * thread dumping when stuck requests discovered (unix only) request.tracking.interval : Frequency (ms) of stuck
   * request inspection request.tracking.threshold : Threshold (ms) before requests are considered "stuck" debug.hops :
   * Log session hopping (ie. processing of session by more than one VM) debug.hops.interval : Number of hops between
   * debug printing debug.invalidate : Log session invalidation vhosts.excluded : comma separated list of virtual hosts
   * that should never use Terracotta clustered sessions (tomcat only) session.debug.sessions : output additional debug
   * information when sessions are looked up, created, etc
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : tcCom l2.tccom.workerthreads - Number of workers threads for network communications. Defaults to
  ...
  /*********************************************************************************************************************
   * Section : CVT cvt.retriever.notification.interval - Interval between log file messages when the CVT retriever is
   * running (in seconds) cvt.statistics.logging.interval - Interval between logging of statistics data (in seconds).
  ...
  /*********************************************************************************************************************
   * Section : Message Batching { server(l2)->clients(l1), clients(l1)->server(l2) } batch.enabled - If true, message
   * batching is enabled. batch.threshold - threshold of comm queue length to trigger message batching
   ********************************************************************************************************************/
  public static final String   L1_BATCH_TXN_ACK_ENABLED                               = "l1.batch.transaction.ack.enabled";
  public static final String   L1_BATCH_TXN_ACK_THRESHOLD                             = "l1.batch.transaction.ack.threshold";

  public static final String   L2_BATCH_TXN_ACK_ENABLED                               = "l2.batch.transaction.ack.enabled";
  public static final String   L2_BATCH_TXN_ACK_THRESHOLD                             = "l2.batch.transaction.ack.threshold";

  /*********************************************************************************************************************
   * Section : TCMessage debug monitoring tcm.monitor.enabled - if enabled the count and size of TC messages will be
   * collected and logged tcm.monitor.delay - the delay (in seconds) between reporting to the log
   ********************************************************************************************************************/
  ...
  /*********************************************************************************************************************
   * Section : HTTP http.defaultservlet.enabled - If true, will serve files through embedded HTTP server
