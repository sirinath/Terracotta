...
public class ManagedObjectStateUtil {
  ...
  // TODO::Figure a way to read from config once its moved there
  private static final boolean OFFHEAP_ENABLED = TCPropertiesImpl.getProperties()
                                                   .getBoolean(TCPropertiesConsts.L2_OFFHEAP_CACHE_ENABLED);
  private static final boolean CACHE_ENTRIES   = TCPropertiesImpl
                                                   .getProperties()
                                                   .getBoolean(
                                                               TCPropertiesConsts.EHCACHE_STORAGESTRATEGY_DCV2_OFFHEAP_CACHEENTRIES_ENABLED);
  ...
  public static ManagedObjectCacheStrategy getCacheStragety(final ManagedObjectState state) {
    ...
    } else if (OFFHEAP_ENABLED && !CACHE_ENTRIES && type == ManagedObjectState.TDC_SERIALIZED_ENTRY
