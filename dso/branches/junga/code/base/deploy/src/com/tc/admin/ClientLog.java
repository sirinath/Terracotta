...
public class ClientLog extends LogPane {
  ...
  public ClientLog(ApplicationContext appContext, IClient client) {
    super();
    error = appContext.getString("log.error");
    warn = appContext.getString("log.warn");
    info = appContext.getString("log.info");
    this.client = client;
    client.addLogListener(new ClientLogListener());
    setEditable(false);
  ...
  class ClientLogListener implements LogListener {
  ...
  public void tearDown() {
    // client.removePropertyChangeListener(serverListener);
    // client.removeServerLogListener(logListener);
