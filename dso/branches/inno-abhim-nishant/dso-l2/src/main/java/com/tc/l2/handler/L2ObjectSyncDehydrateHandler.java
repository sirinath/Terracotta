...
public class L2ObjectSyncDehydrateHandler extends AbstractEventHandler {
  ...
  private static final TCLogger      logger                          = TCLogging
                                                                         .getLogger(L2ObjectSyncDehydrateHandler.class);
  ...
  public void handleEvent(final EventContext context) {
    if (context instanceof ManagedObjectSyncContext) {
      doSyncObjectsDehydrate((ManagedObjectSyncContext) context);
    } else {
  ...
  private void doSyncObjectsDehydrate(ManagedObjectSyncContext context) {
    // logger.info("dehydrating " + context);
    ...
    for (; i.hasNext()
           && L2_OBJECT_SYNC_MESSAGE_MAXSIZE > (out.getBytesWritten() + serializer.getApproximateBytesWritten());) {
    ...
    // logger.info("done dehydrating " + context);
  }
