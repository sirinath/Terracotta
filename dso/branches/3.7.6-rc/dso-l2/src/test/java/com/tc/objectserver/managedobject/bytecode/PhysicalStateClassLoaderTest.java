...
public class PhysicalStateClassLoaderTest extends TCTestCase {
  ...
  public void testHasNoReferences() throws Exception {
    byte[] clazzBytes = loader.createClassBytes(cs, Arrays.asList(FieldType.create("foo", 1, false, 1)));
    Class clazz = loader.defineClassFromBytes(cs.getGeneratedClassName(), 0, clazzBytes, 0, clazzBytes.length);
    ManagedObjectState mos = (ManagedObjectState) clazz.newInstance();
    Assert.assertFalse(mos.hasNoReferences());
  }
