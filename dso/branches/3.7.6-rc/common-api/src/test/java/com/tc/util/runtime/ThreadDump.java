...
public class ThreadDump {
  ...
  private static void doJrcmd(PID pid) {
    ...
    if (jrcmd.isFile()) {
      try {
        Result result = Exec.execute(new String[] { jrcmd.getAbsolutePath(), String.valueOf(pid.getPid()), "print_threads" });
  ...
  static Set<PID> findAllJavaPIDs() {
    ...
    Result result;
    try {
      result = Exec.execute(new String[] { jpsCmd.getAbsolutePath(), "-lmv"});
    } catch (Exception e) {
