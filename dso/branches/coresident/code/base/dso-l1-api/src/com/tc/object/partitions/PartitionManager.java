...
public class PartitionManager {
  ...
  private static Object getCurrentObjectManager() {
    ...
    if (clusterId == null) throw new TCNoPartitionError(
                                                        "Thread calling TC operation does not have partition context Set\n"
  ...
  public static void assertSamePartition(TCObject tco) {
    if (!ClassProcessorHelper.USE_PARTITIONED_CONTEXT) return;
    if (!(getCurrentObjectManager() == tco.getTCClass().getObjectManager())) throw new AssertionError(
                                                                                                      "Data of a different partition can not be joined/modified"
